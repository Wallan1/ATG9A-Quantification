...
plotme<-function(dat,yaxis,labelme,mytitle){
   ggplot(dat,aes(x=Type,y=yaxis,fill=Type, pattern=Rescue)) + 
      #geom_boxplot(width=.7,position = position_dodge(0.9),alpha=0.5, outlier.shape = 20, outlier.size = -1) +
    #geom_jitter(position = position_jitterdodge(1.4),alpha=.6, color = "grey") +
    geom_jitter(position = position_jitterdodge(jitter.width=0.6, dodge.width=0.8),  alpha=0.8, color= "dark grey") +
   stat_boxplot(geom = "errorbar", width = 0.4, position = position_dodge(width = 0.8)) +
    geom_boxplot_pattern(
    aes(),width=0.65,position = position_dodge(0.8), stat= "boxplot", outlier.shape=NA, alpha=0.5,
     ...
     ggtitle(mytitle)+
     theme_minimal()+
  scale_y_continuous(name=labelme,limits = c(0.24, 0.84))+
     theme(plot.title=element_text(hjust=.5))+
        scale_fill_viridis(discrete=TRUE, end=.45) +
        scale_color_viridis(discrete=TRUE,end=.45)+
        scale_pattern_manual(values = c(unt = "none", WTtep = "stripe", LIRless = "circle", Cterm = "weave"))
