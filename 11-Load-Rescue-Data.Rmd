...
*change input file path*
```{r}
temp=read.csv(file="./rescue-analysis_22May22/datasets/NSW4046_31Aug22.csv",header=TRUE)
temp$BvsS<-"Basal"
temp$Rep<-1
nobs=nrow(temp)
for (i in 1:nobs) {
...
rs2<-temp[which(temp$meanMyc>130),c(1,4,6)]
...
for (c in 1:ntemp) {
  ...
  else if (!is.na(included$pCellmean[c]) && included$pCellmean[c]>130)
 {
    included$pinclude[c]<-"Y"
  }
  ...
  else if (!is.na(included$gCellmean[c]) && included$gCellmean[c]>130)
 {
    included$ginclude[c]<-"Y"
  }
