...
*change input file path*
```{r}
temp=read.csv(file="./rescue-analysis_22May22/datasets/2023/NSW4046_analysis_4Nov23.csv",header=TRUE)
temp$BvsS<-"Basal"
temp$Rep<-1
nobs=nrow(temp)
for (i in 1:nobs) {
...
for (c in 1:ntemp) {
  if (substr(temp[c,1],46,48)=="000") {
    temp$Rescue[c]<-"unt"
  }
  else if (substr(temp[c,1],46,48)=="087") {
    temp$Rescue[c]<-"WTtep"
  }
    else if (substr(temp[c,1],46,48)=="088") {
...
#whole cell revised 2023
whole<-temp[which(!is.na(temp$CellID)),c(1:34)]

whole$include<-NA
ntemp<-nrow(whole)

for (c in 1:ntemp) {
  if (substr(whole[c,1],48,48)=="0") {
    whole$include[c]<-"Y"
  }
  else if (whole$meanMyc[c]>164) {
    whole$include[c]<-"Y"
  }
...
rs2<-temp[which(temp$meanMyc>110),c(1,4,6)]
...
for (c in 1:ntemp) {
  ...
  else if (!is.na(included$pCellmean[c]) && included$pCellmean[c]>110)
 {
    included$pinclude[c]<-"Y"
  }
  ...
  else if (!is.na(included$gCellmean[c]) && included$gCellmean[c]>110)
 {
    included$ginclude[c]<-"Y"
  }
...
Get Sum of cell counts included per type
*INCLUDED CELLS NOT TOTAL IN IMAGE*
```{r}
wTidy$IncCellSum<-NA
ntemp<-nrow(wTidy)
...
for (t in c("WT", "Control", "tepsin")) {
  for (tr in c("unt", "WTtep", "LIRless", "Cterm")) {
    for (r in c("1", "2","3", "4")) {
       wTidy$IncCellSum[which(wTidy$Type==t & wTidy$Rescue==tr & wTidy$Rep==r)]<-sum(wTidy$include[which(wTidy$Type==t & wTidy$Rescue==tr & wTidy$Rep==r)]=="Y",na.rm = TRUE)
      }
    }
  }
```
...
#Export dataframe
```{r}
write.csv(wTidy, file= "./rescue-analysis_22May22/datasets/2023/NSW4046_tidydata-Myc164sort_4Nov23GA3.csv")
```
